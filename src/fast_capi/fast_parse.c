...
bool fp_report_type_error(const FastParser *fp, size_t index, PyObject *val) {
    ...
    const char *expected_type = spec->type_name ? spec->type_name : 
                                (spec->type_guard ? spec->type_guard->tp_name : "unknown");

    PyErr_Format(PyExc_TypeError, "argument '%s' must be %s, not %.200s", 
                 spec->name, expected_type, Py_TYPE(val)->tp_name);
    return false;
}
...
[[nodiscard]] bool 
fp_parse_legacy(PyObject *args, PyObject *kwargs, PyObject *unused,
